...
function ShapePreview({ shape, prodType, active }: { shape: ShapeType; prodType: ProdType; active: boolean }) {
  ...
  if (shape === 'rect' && prodType === 'door') {
    ...
    const s = Math.min(ow, oh);
    const sx = ox + (ow - s) / 2, sy = oy + (oh - s) / 2;
    outerPath = `M${sx},${sy} L${sx + s},${sy} L${sx + s},${sy + s} L${sx},${sy + s} Z`;
    const b2 = band * 0.8;
    innerPath = `M${sx + b2},${sy + b2} L${sx + s - b2},${sy + b2} L${sx + s - b2},${sy + s - b2} L${sx + b2},${sy + s - b2} Z`;
  } else if (shape === 'arch') {
...
function Step4({ store, setField }: any) {
  ...
  function setOkantovka(side: 'Top' | 'Bottom' | 'Left' | 'Right', val: string) {
    const n = Math.max(0, Math.min(300, parseInt(val) || 0));
    if (linkedOk) {
    ...
    <div className="flex flex-col gap-5 px-4 py-4">
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {options.map((opt) => {
            const active = store.openingType === opt.value;
            return (
              <button
                key={opt.value}
                type="button"
                onClick={() => setField('openingType', opt.value)}
      ...
      <div>
        ...
        {linkedOk ? (
          <input type="number" value={store.okantovkaTop} min={0} max={300}
          ...
          <div className="grid grid-cols-2 gap-2">
            {(['Top','Bottom','Left','Right'] as const).map((side) => (
              <div key={side}>
                ...
                <input type="number" value={store[`okantovka${side}`]} min={0} max={300}
      ...
      <div>
        ...
        <input
          type="number"
          value={store.luverSpacing}
          min={50} max={1000} step={10}
          onChange={(e) => {
            const n = Math.max(50, Math.min(1000, parseInt(e.target.value) || 300));
